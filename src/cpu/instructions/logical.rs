...
impl SCpu {
	...
	pub fn exe_bit_imm(&mut self, data: u16) {
		self.status.set(StatusFlags::Zero, (self.get_acc() & data) == 0);
	}
	...
	pub fn exe_bit(&mut self, data: u16) {
		...
		let (n, v) = match self.status.contains(StatusFlags::Accumulator8bit) {
			true => (7, 6),
			false => (15, 14),
		};
		// These flags are set depending on data, NOT the result of the and operation
		self.status.set(StatusFlags::Overflow, (data >> v) == 1);
		self.status.set(StatusFlags::Negative, (data >> n) == 1);
		
		let and = self.get_acc() & data;
		self.status.set(StatusFlags::Zero, and == 0);
	}
	...
	pub fn exe_cmp(&mut self, data: u16) {
		...
		self.status.set(StatusFlags::Carry, cmp <= 0);
		self.status.set(StatusFlags::Zero, cmp == 0);
		self.status.set(StatusFlags::Negative, (cmp as i8) < 0);
	}
	...
	pub fn exe_rola(&mut self) {
		...
		let c = self.carry();
		self.status.set(StatusFlags::Carry, (self.get_acc() & 0x01) == 1);
		self.set_acc((self.get_acc() << 1) | c);
		self.status.set(StatusFlags::Zero, self.get_acc() == 0);
		self.status.set(StatusFlags::Negative, (self.get_acc() >> b) == 1);
	}
	...
	pub fn exe_ror(&mut self, low_addr: u32, high_addr: u32) {
		match self.status.contains(StatusFlags::Accumulator8bit) {
			true => {
				let mut val = self.mem_read(low_addr);
				let c = self.carry() as u8;
				self.status.set(StatusFlags::Carry, bit_set!(val, 01));
				val = (val >> 1) | (c << 7);
				self.status.set(StatusFlags::Zero, val == 0);
				self.status.set(StatusFlags::Negative, bit_set!(val, 7));
				self.mem_write(low_addr, val);
